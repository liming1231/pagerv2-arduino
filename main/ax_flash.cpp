...
bool read_flash_config(void)
{
  ...
  else
  {
    ...
    Serial.printf("\tnvs_open failed: %s\n", esp_err_to_name(err));
    switch (err)
    {
    case ESP_ERR_NVS_NOT_INITIALIZED:
      Serial.println("\tNVS UnInitialized");
      break;
    ...
    case ESP_ERR_NVS_INVALID_NAME:
      Serial.println("\tNVS Invalid Name");
      break;
    default:
      Serial.printf("\tOther Error: 0x%x\n", err);
    }
    return false;
  }
}
    ...
    if (flushTimestep - startupTimestep <= 2000)
    {
      if (digitalRead(DECALL_BUTTON) == LOW)
      {
        while (true)
        {
          ...
          if (digitalRead(DECALL_BUTTON) == LOW) // 测试，在使用中要改回去HIGH
          {
            break;
          }
...
bool update_ap_gw(void)
{
  ...
  if (nvs_open("storage", NVS_READWRITE, &handle) == ESP_OK)
  {
    nvs_erase_key(handle, AP_GATEWAY);
    delay(1);
    nvs_set_u8(handle, AP_GATEWAY, ap_gw_select_flag);
    delay(1);
    nvs_commit(handle);
    delay(1);
    nvs_close(handle);
...
bool update_dns(void)
{
  ...
  if (nvs_open("storage", NVS_READWRITE, &handle) == ESP_OK)
  {
    ...
    nvs_erase_key(handle, DNS_CFG_FLAG);
    delay(1);
    nvs_set_u32(handle, DNS_DATA, dnsUpdateVar.ip_u32);
    delay(1);
    nvs_set_u8(handle, DNS_CFG_FLAG, dnsUpdateVar.ipCfgFlag);
    delay(1);
    nvs_commit(handle);
    delay(1);
    dnsUpdateVar.ip_u32 = 0;
    nvs_get_u32(handle, DNS_DATA, &dnsUpdateVar.ip_u32);
    Serial.printf("dnsUpdateVar.dns: %d\r\n", dnsUpdateVar.ip_u32);
    Serial.printf("dnsUpdateVar.dnsCfgFlag: %d\r\n", dnsUpdateVar.ipCfgFlag);
    delay(1);
    nvs_close(handle);
    return true;
  }
...
bool update_wifi_staticip(void)
{
  ...
  if (nvs_open("storage", NVS_READWRITE, &handle) == ESP_OK)
  {
    if (!cfgWifi.Static_ip)
    {
      ...
      nvs_erase_key(handle, STTC_SNT);
      delay(1);
      nvs_set_u8(handle, IS_STATICIP, varData.static_ip);
      delay(1);
      nvs_commit(handle);
      delay(1);
      nvs_close(handle);
    ...
    else
    {
      ...
      nvs_set_u8(handle, IS_STATICIP, varData.static_ip);
      delay(1);
      nvs_set_u32(handle, STTC_IP, staticIpUpdateVar.ip_u32);
      delay(1);
      nvs_set_u32(handle, STTC_GW, staticGwUpdateVar.ip_u32);
      delay(1);
      nvs_set_u32(handle, STTC_SNT, staticSbntUpdateVar.ip_u32);
      delay(1);
      nvs_set_u8(handle, DNS_CFG_FLAG, dnsUpdateVar.ipCfgFlag);
      delay(1);
      nvs_set_u32(handle, DNS_DATA, dnsUpdateVar.ip_u32);
...
bool update_wifi(void)
{
  ...
  if (nvs_open("storage", NVS_READWRITE, &handle) == ESP_OK)
  {
    ...
    nvs_commit(handle);
    delay(1);
    nvs_set_str(handle, WIFI_NAME, varData.wifiName);
    delay(1);
    nvs_set_str(handle, WIFI_PWD, varData.wifiPwd);
    delay(1);
    nvs_commit(handle);
    delay(1);
    nvs_close(handle);
...
bool update_wpa2_enterprise_wifi(void)
{
  ...
  if (nvs_open("storage", NVS_READWRITE, &handle) == ESP_OK)
  {
    ...
    nvs_erase_key(handle, STTC_SNT);
    delay(1);
    if (nvs_set_str(handle, WIFI_NAME, varData.wifiName) != ESP_OK)
    {
      nvs_close(handle);
      return false;
    }
    if (nvs_set_str(handle, WIFI_PWD, varData.wifiPwd) != ESP_OK)
    {
      nvs_close(handle);
      return false;
    }
    if (nvs_set_str(handle, WIFI_PROTO, varData.wifiProto) != ESP_OK)
    {
      nvs_close(handle);
      return false;
    }
    if (nvs_set_u8(handle, WIFI_METHOD, varData.wpa2_auth_method) != ESP_OK)
    {
      nvs_close(handle);
      return false;
    }
    if (nvs_set_str(handle, WIFI_WPA2_ID, varData.wpa2_identity) != ESP_OK)
    {
      nvs_close(handle);
      return false;
    }
    if (nvs_set_str(handle, WIFI_WPA2_USR, varData.wpa2_username) != ESP_OK)
    {
      nvs_close(handle);
      return false;
    }
...
bool update_privet_deploy_vars(void)
{
  ...
  if (nvs_open("storage", NVS_READWRITE, &handle) == ESP_OK)
  {
    ...
    nvs_erase_key(handle, DIS_LANG);
    delay(1);
    if (nvs_set_str(handle, PRIV_PROTO, varData.priv_proto) != ESP_OK)
    {
      nvs_close(handle);
      return false;
    }
    if (nvs_set_str(handle, PRIV_DEPLOY, varData.priv_addr) != ESP_OK)
    {
      nvs_close(handle);
      return false;
    }
    if (nvs_set_i32(handle, PRIV_DEPLOY_PORT, varData.priv_port) != ESP_OK)
    {
      nvs_close(handle);
      return false;
    }
    if (nvs_set_str(handle, PRIV_DEPLOY_USR, varData.priv_usr) != ESP_OK)
    {
      nvs_close(handle);
      return false;
    }
    if (nvs_set_str(handle, PRIV_DEPLOY_PWD, varData.priv_pwd) != ESP_OK)
    {
      nvs_close(handle);
      return false;
    }
    varData.envType = ENV_TYPE::PRIVAT;
    if (nvs_set_u8(handle, ENV_TYPE_FLASH, varData.envType) != ESP_OK)
    {
      nvs_close(handle);
      return false;
...
bool update_env_type(void)
{
  ...
  if (nvs_open("storage", NVS_READWRITE, &handle) == ESP_OK)
  {
    ...
    nvs_erase_key(handle, IS_AX_ENV);
    delay(1);
    nvs_set_u8(handle, DIS_LANG, displayVars.user_lang);
    delay(1);
    nvs_set_u8(handle, IS_AX_ENV, varData.isAxEnv);
    delay(1);
    nvs_set_u8(handle, ENV_TYPE_FLASH, varData.envType);
    delay(1);
    nvs_close(handle);
    return true;
  }
...
bool update_web_display_lang(void)
{
  ...
  if (nvs_open("storage", NVS_READWRITE, &handle) == ESP_OK)
  {
    nvs_erase_key(handle, WEB_DIS_LANG);
    delay(1);
    nvs_set_u8(handle, WEB_DIS_LANG, displayVars.web_display_lang);
    delay(1);
    nvs_commit(handle);
    delay(1);
    nvs_close(handle);
